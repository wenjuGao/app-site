<template>
	<div :class="`dropdown  dropdown-end menu-swap`" ref="menuRef">
		<label tabindex="0" class="btn btn-circle swap swap-rotate">
			<input type="checkbox" :checked="flag" @click="tiggerChange" />
		...
		<ul tabindex="0" class="menu bg-base-200 shadow-md w-56 p-0 [&_li>*]:rounded-none dropdown-content rounded-box">
			<li v-for="(item, index) in props.menu" :key="index">
				<a :href="item.link" aria-current="page">{{ item.label }}</a>
			</li>
		</ul>
	</div>
</template>
...
const tiggerChange = () => {
	console.log('tiggerChange', flag.value)
...
onClickOutside(menuRef, () => {
	console.log('onClickOutside', menuRef)
	flag.value = false
	menuRef.value?.blur()
})
</script>
